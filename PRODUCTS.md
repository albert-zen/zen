...
| 10   | ZenX Plugin Platform：Tool Environment、Plugin Host/Runtime、通用 UI/ZAS 生命周期 | Public SDK/create/validate/pack、统一 Package lifecycle/Runtime/SDK、渐进发现、structured result renderer、桌面 AppServer composition、Generic UI Host、ZAS lifecycle、完整 profile source/lifecycle matrix、五个第一方 packages 与统一 Marketplace inventory 已完成纵向切片 |
