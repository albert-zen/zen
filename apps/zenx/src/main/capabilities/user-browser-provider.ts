...
interface UserBrowserAttachmentClosure {
  targetId: string;
  sessionId?: string;
...
class ZenXUserBrowserOperationClosure {
...
export class UserBrowserCdpBackend implements ZenXBrowserBackend {
  ...
  readonly #closure = new ZenXUserBrowserOperationClosure();
  ...
  async open(
    ...
    const operation = this.#startSessionOperation(session, async () => {
      ...
      const existingOwner = this.#tabs.get(targetId)?.ownerSessionId;
      if (existingOwner !== undefined && existingOwner !== session.id) {
        this.#recordUnresolvedCreate(session, pendingUrl, beforeTargetIds);
      ...
      if (
        ...
        (this.#tabs.get(createdMatches[0]?.targetId ?? "")?.ownerSessionId ??
  ...
  async #reconcileCreate(
    ...
    if (
      ...
      (this.#tabs.get(candidates[0]?.targetId ?? "")?.ownerSessionId ??
        session.id) !== session.id
    ) {
...
class JsonRpcUserBrowserCdpClient implements UserBrowserCdpClient {
  ...
  >();
  readonly #attachmentClosures = new Map<
    string,
    UserBrowserAttachmentClosure
  >();
  ...
  #attachmentClosure(targetId: string): UserBrowserAttachmentClosure {
    let closure = this.#attachmentClosures.get(targetId);
  ...
  async #compensateLateAttach(
    ...
    if (typeof sessionId !== "string") {
      closure.taint = "late attach response was invalid";
      return;
    }
    closure.attach = "known-success";
    closure.sessionId = sessionId;
  ...
  #receiveEvent(message: Record<string, unknown>): void {
    ...
    if (
      ...
      if (targetId !== undefined) this.#reapTarget(targetId, true);
      return;
    }
    if (
