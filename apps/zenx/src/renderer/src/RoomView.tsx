  ...
  const create = async () => {
    ...
    try {
      ...
      const created = next.rooms.find(
        (room) =>
          room.name === name.trim() && room.members.length === members.length,
      );
      setName("");
      setMembers([]);
      setCreating(false);
      ...
      <div className="page-scroll">
        <div className="page-intro">
          <div>
            ...
            <p>
              A Room carries shared conclusions and routes explicit @mentions
              into ordinary member Turns. Reasoning and tools stay in the source
              Thread.
            </p>
          </div>
        </div>
                ...
                <span key={member.threadId}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setMembers((current) =>
                        current.filter(
                          (item) => item.threadId !== member.threadId,
                        ),
                      )
                    }
                  >
        ...
        <div className="section-heading">
          <h2>Your Rooms</h2>
          <span>{snapshot.rooms.length} local</span>
        </div>
        <div className="room-grid">
          {snapshot.rooms.length === 0 ? (
            <div className="page-card page-empty">
              No Rooms yet. Create one and bind named members to existing
              Threads.
            </div>
          ) : (
            snapshot.rooms.map((room) => (
              <button
                className="page-card room-card"
                type="button"
                key={room.id}
...
function ActiveRoom({
  ...
  room: TriggerSnapshot["rooms"][number];
  threads: readonly NativeThreadSummary[];
  onBack(): void;
  onOpenThread(id: string): void;
  onOpenSidebar?(): void;
  ...
  const availableThreads = threads.filter(
    (thread) =>
      ...
      !room.members.some((member) => member.threadId === thread.threadId),
  );
  const [memberThread, setMemberThread] = useState(
    availableThreads[0]?.threadId ?? "",
  );
  ...
  const send = async () => {
    ...
    setSendError(null);
    try {
      await window.zenx.triggers.postRoomMessage(room.id, "You", draft.trim());
      setDraft("");
    } catch (reason) {
      setSendError(
        `${describeError(reason)} Draft was kept; retry explicitly.`,
      );
    } finally {
      setSending(false);
    }
  ...
  const addMember = async () => {
    ...
    try {
      ...
      setMemberError(`${describeError(reason)} Member draft was kept.`);
    } finally {
    ...
    <section className="product-page active-room">
      <header className="page-header">
        <div className="page-title">
          ...
          <button
            className="quiet-button room-back"
            type="button"
            onClick={onBack}
          >
            Rooms
          ...
          {room.members.map((member) => (
            <span key={member.threadId}>
              ...
              <button
                ...
                onClick={() => {
                  setMemberPending(member.threadId);
                  setMemberError(null);
                  void window.zenx.triggers
                    .removeRoomMember(room.id, member.threadId)
                    .catch((reason: unknown) =>
                      setMemberError(describeError(reason)),
                    )
