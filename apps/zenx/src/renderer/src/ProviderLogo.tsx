...
const formalProviderAssets: Partial<Record<ProviderLogoKind, string>> = {
  ...
  siliconflow: new URL("./assets/providers/siliconflow.svg", import.meta.url)
    .href,
...
export function providerLogoKindForIdentity(
  ...
  if (identity.includes("siliconflow") || identity.includes("硅基流动"))
    return "siliconflow";
  if (identity.includes("dashscope") || identity.includes("百炼"))
    return "dashscope";
  ...
  if (identity.includes("openai") || /^gpt-/iu.test(modelOrDisplayName))
    return "openai";
