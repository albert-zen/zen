...
The runner accepts browser tasks whose ids match `B<number>` in `tasks.json`, including newly added tasks; it does not hardcode the current list. It creates a fresh ZenX thread and unique Browser session hint for every trial and runs repeated trials sequentially to separate measurements. The default single trial is exploratory. Use at least three fresh trials for reliability comparisons.

### Concurrent agents

To test isolation, launch two runner processes against the same ZenX descriptor and fixture at the same time, with separate new output directories. Use B1 in both processes for a like-for-like workload, or pair B1 with B6 for different page interactions. Each process must retain its own thread, task receipts and final observation. Both must complete without resource-busy retry loops or another thread's wait handle. Record overlapping tool intervals as well as the two final outcomes; two successful but sequential turns do not establish concurrency.

Browser calls coordinate by actual thread/session and page ownership. Different pages can be admitted together; the default Playwright CLI still serializes commands within one provider session because it uses a selected page. Separate agents have separate provider sessions even when they choose the same public session name. Same-page actions and whole-session closure queue rather than requiring the model to poll a conflicting thread. Desktop operations retain a shared desktop lock, independent of Browser. Queue cancellation and session-close ordering are deterministic host/core regression cases, not inferred from a model's final answer.
