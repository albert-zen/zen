...
test("repairs persisted packaged-artifact workspace to an explicit safe folder", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "zenx-safe-cwd-"));
  ...
  const safe = path.join(directory, "Documents");
  try {
    const first = settingsForWithoutSubscription(directory);
    await first.initialize({ ZENX_CWD: unsafe });
    const second = settingsForWithoutSubscription(directory);
    await second.initialize({}, { safeWorkspace: safe });

    const profile = (await second.publicSettings()).profile;
    assert.equal(profile.workspace, path.resolve(safe));
    assert.deepEqual(profile.workspaces, [path.resolve(safe)]);
    assert.equal((await second.hostConfig()).cwd, path.resolve(safe));
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});

test("safe first-launch workspace selection skips packaged artifact paths", () => {
  ...
  assert.equal(
    resolveSafeWorkspace([artifact, documents]),
    path.resolve(documents),
  );
  assert.throws(
    () => resolveSafeWorkspace([artifact]),
