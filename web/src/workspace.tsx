...
export function AgentWorkspace(props: AgentWorkspaceProps = {}): React.ReactElement {
  ...
  const initialMode: RuntimeMode =
    props.initialMode ?? (params.get('mode') === 'demo' ? 'demo' : 'real');
  const [mode, setMode] = React.useState<RuntimeMode>(initialMode);
  const [client, setClient] = React.useState(() => createClient(initialMode, () => undefined));
  ...
  React.useEffect(() => {
    ...
    void Promise.resolve()
      .then(reconnect)
      .catch((cause) => {
        if (!disposed) setStreamError(readError(cause));
      });
    return () => {
      disposed = true;
      client.dispose();
  ...
  function changeMode(nextMode: RuntimeMode): void {
    ...
    setMode(nextMode);
    setClient(
      createClient(nextMode, (status, error) => {
        ...
        <header className="flex items-center justify-between gap-4 border-b border-zinc-800 bg-zinc-950 px-6 max-md:px-4">
          <div className="min-w-0">
            <h1 className="truncate text-base font-bold">
              {activeThread
                ? threadTitle({ id: activeThread.id, items: state.items })
                : 'No thread'}
            </h1>
                ...
                <div className="grid min-h-[calc(100vh-220px)] place-items-center text-center text-zinc-400">
                  <div>
                    ...
                    <p className="max-w-md leading-7">
                      Ask Zen to inspect code, run commands, or explain the item timeline. Threads
                      stay on the left so the conversation gets the room.
                    </p>
                  </div>
                </div>
              )}
...
function TimelineMessage({
  ...
  return (
    <article
      className={cn('grid gap-2', side === 'end' ? 'justify-items-end' : 'justify-items-start')}
    >
    ...
    return (
      <div>
        <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold uppercase text-zinc-400">
          <span>
            Shell {row.exitCode === undefined ? row.status : `${row.status} | exit ${row.exitCode}`}
          </span>
...
function OutputBlock({
  ...
  return (
    <div className="mt-3">
      <div
        className={cn(
          'mb-1 text-[11px] font-bold uppercase text-zinc-500',
...
function threadTitle(thread: {
  id: string;
  items: Iterable<ThreadSnapshot['items'][number]>;
}): string {
  return summarize(latestContent(thread.items, 'user.message.completed') ?? thread.id, 48);
}

function latestAssistantOrUser(
  items: Iterable<ThreadSnapshot['items'][number]>
): string | undefined {
  return (
    latestContent(items, 'assistant.message.completed') ??
    latestContent(items, 'user.message.completed')
  );
}
...
function latestItemTimestamp(
  items: readonly ThreadSnapshot['items'][number][]
): number | undefined {
