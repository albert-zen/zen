...
Publication is not evidence that its conclusions are correct. The Host verifies IDs and lineage, not semantic truth. Component source reads use the shared public Item projection: opaque payloads are removed recursively, while public text and summaries remain available. The trusted native source inspector retains the full canonical Item. The existing iframe sandbox forbids same-origin access; the production parent CSP and frame CSP together enforce the network/navigation boundary. Host commands/navigation are denied. Scripts may still exhaust browser resources; this is not a separate-process availability sandbox.
