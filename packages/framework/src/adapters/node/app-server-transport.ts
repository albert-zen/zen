...
function resolveRequestTimeout(timeoutMs: number | undefined): number {
  const resolved = timeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS;
  if (!Number.isFinite(resolved) || resolved <= 0 || resolved > MAX_REQUEST_TIMEOUT_MS) {
    throw new Error(
      `App Server HTTP client request timeout must be finite, positive, and no greater than ${MAX_REQUEST_TIMEOUT_MS}ms`
    );
  }
  return resolved;
}
...
export class HttpAppServerClient implements AppServerClient {
  ...
  private readonly operations = new Set<HttpClientOperation>();
  ...
  async request(request: AppServerRequestInput): Promise<AppServerResponse> {
    return await this.withClientOperation(
      (signal) => this.withSubscriptionsReady(() => this.requestDirect(request, signal)),
  ...
  async close(): Promise<void> {
    ...
    this.closed = true;
    const closeError = this.clientClosedError();
    for (const subscription of [...this.subscriptions]) {
      this.stopSubscription(subscription, closeError);
    }
    for (const operation of [...this.operations]) operation.cancel(closeError);
  ...
  private async requestDirect(
    ...
    if (signal === undefined) {
      return await this.withClientOperation(
        (operationSignal) => this.requestDirect(request, operationSignal),
        'HTTP_REQUEST_TIMEOUT'
      );
    }
    if (signal.aborted) {
      throw signal.reason instanceof Error ? signal.reason : this.clientClosedError();
    }

    const response = await Promise.resolve().then(() =>
      this.fetchImpl(new URL(REQUEST_PATH, this.baseUrl), {
        method: 'POST',
        headers: {
  ...
  subscribe(listener: AppServerNotificationListener): AppServerSubscription {
    ...
    return () =>
      this.stopSubscription(subscription, new Error('HTTP event subscription disconnected'));
  }

  private stopSubscription(subscription: HttpSubscriptionState, cause: Error): void {
    if (!subscription.active) return;
    subscription.active = false;
    subscription.generation += 1;
    subscription.phase = 'closed';
    subscription.gate.reject(cause);
    subscription.controller.abort(cause);
  ...
  private async consumeEventStream(
    ...
    const connection = this.startClientOperation('SSE_CONNECT_TIMEOUT');
    ...
    try {
      const fetchPromise = Promise.resolve().then(() =>
        this.fetchImpl(new URL(EVENTS_PATH, this.baseUrl), {
          headers: {
      ...
      const response = await Promise.race([fetchPromise, connection.cancellation]);
      connection.finish();
  ...
  private async withClientOperation<T>(
    operation: (signal: AbortSignal) => Promise<T>,
    timeoutCode: 'HTTP_REQUEST_TIMEOUT' | 'SSE_CONNECT_TIMEOUT'
  ): Promise<T> {
    const clientOperation = this.startClientOperation(timeoutCode);
    try {
      return await Promise.race([
        Promise.resolve().then(() => operation(clientOperation.controller.signal)),
        clientOperation.cancellation,
      ]);
    } finally {
      clientOperation.finish();
    }
  }

  private startClientOperation(
    timeoutCode: 'HTTP_REQUEST_TIMEOUT' | 'SSE_CONNECT_TIMEOUT'
  ): HttpClientOperation {
    if (this.closed) throw this.clientClosedError();

    let rejectCancellation!: (cause: AppServerTransportError) => void;
    const controller = new AbortController();
    const cancellation = new Promise<never>((_resolve, reject) => {
      rejectCancellation = reject;
    });
    const operation: HttpClientOperation = {
      controller,
      cancellation,
      cancel: (cause) => {
      ...
      timeout: setTimeout(() => {
        operation.cancel(
          new AppServerTransportError(
            timeoutCode,
            `App Server HTTP client operation timed out after ${this.requestTimeoutMs}ms`
          )
        );
  ...
  private clientClosedError(): AppServerTransportError {
    return new AppServerTransportError('CLIENT_CLOSED', 'App Server HTTP client is closed');
  }
}
...
type HttpClientOperation = {
  ...
  readonly cancel: (cause: AppServerTransportError) => void;
...
async function readServerSentEvents(
  ...
  try {
    while (!signal.aborted) {
      const result = await readServerSentEventChunk(reader, signal);
...
function readServerSentEventChunk(
  ...
  return new Promise((resolve, reject) => {
    let settled = false;
    const finish = (result: ReadableStreamReadResult<Uint8Array> | undefined): void => {
      if (settled) return;
      settled = true;
