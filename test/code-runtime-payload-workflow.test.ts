...
for (const environmentSpool of [false, true])
  test(`large JSON is raw and model output bounded (env spool=${environmentSpool})`, async (t) => {
    ...
    const env = new ToolEnvironment({
      ...
      ...(environmentSpool ? { toolOutputSpool: spool } : {}),
    });
    t.after(async () => {
      await env.close();
      await spool.close();
