...
describe('web ui state projection', () => {
  ...
  it('caches snapshots and processes 1k/5k ordered appends with constant work and no sequence copies', () => {
    ...
    expect(workSince(baseline, projection.getWork())).toEqual({
      fastPathOperations: 1000,
      rebuilds: 0,
      sequenceCopies: 0,
  ...
  it('keeps stable shell slots when approval resolution tombstones a row', () => {
    ...
    append(
      item({
        ...
        payload: { toolName: 'shell', input: { command: 'one' } },
      })
    );
    append(
      item({
    ...
    append(
      item({
        ...
        seq: 3,
        type: 'tool.call.started',
        payload: { toolName: 'shell', input: { command: 'two' } },
      })
    );
    append(
      item({
        ...
        payload: { approvalId: 'approval-1', decision: 'approveOnce' },
      })
    );
    append(
      item({
        ...
        targetId: 'shell-2',
        payload: { delta: { stream: 'stdout', chunk: 'two output' } },
      })
    );
    ...
    for (let index = 1; index <= 1000; index += 1) {
      ...
      for (const nextItem of [
        item({
          ...
          seq: ++seq,
          type: 'tool.call.started',
          payload: { toolName: 'shell', input: { command: String(index) } },
        }),
        item({
          id: approvalId,
          seq: ++seq,
        ...
        item({
          ...
          type: 'approval.resolved',
          targetId: shellId,
          payload: { approvalId, decision: 'approveOnce' },
        }),
        item({
          id: `${shellId}-output`,
          seq: ++seq,
          type: 'tool.output.delta',
          targetId: shellId,
          payload: { delta: { stream: 'stdout', chunk: 'x' } },
        }),
        item({
          id: `${shellId}-result`,
          seq: ++seq,
  ...
  it('applies appended item notifications in item sequence order', () => {
    ...
    expect(state.timelineRows.map((row) => row.itemId)).toEqual(['item-1', 'item-2']);
  });
