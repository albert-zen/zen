...
describe('AppServer approval flow', () => {
  it('rejects a mismatched tuple, then records direct approval facts before a declined tool error', async () => {
      ...
      threadManagerOptions: {
        runtimeFactory: ({ approvalBroker }) => ({
          ...
          toolRuntime: new PolicyToolRuntime({
            ...
            policy: {
              evaluate: () => ({ type: 'needsApproval', reason: 'shell requires approval' }),
            },
            toolRuntime: {
              async *execute() {
                executed = true;
    ...
    const turn = await server.request({
      method: 'turn/start',
      params: { threadId: started.result.thread.id, input: 'run' },
    });
    ...
    await terminal;
    const thread = await server.request({
      method: 'thread/read',
      params: { threadId: pending.threadId },
    });
    if (!thread.ok || thread.method !== 'thread/read') throw new Error('thread did not load');
    expect(executed).toBe(false);
    expect(thread.result.thread.items.map((item) => item.type)).toEqual(
      expect.arrayContaining(['approval.requested', 'approval.resolved', 'tool.error'])
    );
    ...
    async *generate() {
      ...
      if (calls === 1) {
        yield {
          ...
          content: 'calling',
          toolCalls: [{ id: 'tool-1', name: 'shell', input: { command: 'echo blocked' } }],
        };
        return;
      }
