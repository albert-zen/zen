...
describe('FileThreadJournal', () => {
  it('preserves non-Error journal failure text for actionable callers', () => {
    const error = new ThreadJournalError('thread-1', 'append', 'disk unavailable');

    expect(error.message).toBe('Thread journal append failed for thread-1: disk unavailable');
    expect(error.cause).toBe('disk unavailable');
  });

  it('uses an Error cause message when a journal operation supplies one', () => {
    expect(new ThreadJournalError('thread-2', 'flush', new Error('sync failed')).message).toBe(
      'Thread journal flush failed for thread-2: sync failed'
    );
  });
    ...
    expect(replay).toEqual(
      expect.objectContaining({
        ...
        items: [
          ...
          expect.objectContaining({ type: 'user.message.completed' }),
        ],
      })
    );
  ...
  it('keeps separate thread queues independent', async () => {
    ...
    let delayed = false;
    const fs = slowFileSystem('slow', barrier, () => {
      delayed = true;
    });
  ...
  it('makes write failures sticky without stopping other threads and aggregates close', async () => {
    ...
    const journal = new FileThreadJournal({ dir, fileSystem: fs });
    await expect(journal.create('broken', created('broken'))).rejects.toBeInstanceOf(
      ThreadJournalError
    );
    await expect(
      journal.append('broken', item('user.message.completed', 'broken', 2))
    ).rejects.toBeInstanceOf(ThreadJournalError);
  ...
  it('recovers a truncated final record and surfaces interior corruption while retaining valid threads', async () => {
    ...
    await writeFile(
      journalPath(dir, 'corrupt'),
      `${JSON.stringify({ version: 1, item: created('corrupt') })}\nnot-json\n`,
      'utf8'
    );
    await writeFile(
      journalPath(dir, 'valid'),
      `${JSON.stringify({ version: 1, item: created('valid') })}\n`,
      'utf8'
    );
  ...
  it('writes 500 deltas as 501 linear records and bytes', async () => {
    ...
    for (let index = 0; index < 500; index += 1)
      await journal.append(
        'bulk',
        item('assistant.message.delta', 'bulk', index + 2, { content: 'x'.repeat(32) })
      );
    await journal.close();
  ...
  it('replays exact multibyte content after injected short writes', async () => {
    ...
    await journal.append(
      'unicode',
      item('assistant.message.completed', 'unicode', 2, { content: '你好, durable journal' })
    );
    await journal.close();
    ...
    expect(result).toMatchObject({
      ...
      items: [
        expect.anything(),
        expect.objectContaining({ payload: { content: '你好, durable journal' } }),
      ],
    });
  });
});

function created(threadId: string): Item {
  return item('thread.created', threadId, 1, { threadId });
}
function item(type: string, threadId: string, seq: number, payload: unknown = {}): Item {
  return {
    id: `${threadId}-${seq}`,
    type,
    createdAtMs: seq,
...
function slowFileSystem(
  threadId: string,
  barrier: ReturnType<typeof deferred<void>>,
  started: () => void
  ...
  return {
    ...
    async open(path, flags) {
      ...
      if (
        path.endsWith(`thread-${Buffer.from(threadId).toString('base64url')}.jsonl`) &&
        flags === 'wx'
      )
        return {
          write: async (buffer: Buffer, position?: number | null) => {
            started();
            await barrier.promise;
            return await handle.write(buffer, position);
          },
          sync: () => handle.sync(),
          close: () => handle.close(),
...
function failingFileSystem(threadId: string): ThreadJournalFileSystem {
  ...
  return {
    ...fs,
    async open(path, flags) {
      if (path.endsWith(`thread-${Buffer.from(threadId).toString('base64url')}.jsonl`))
  ...
  return {
    ...
    async open(path, flags) {
      ...
      if (flags === 'wx' || flags === 'a')
        return {
          write: async (buffer: Buffer, position?: number | null) =>
            await handle.write(buffer.subarray(0, Math.min(2, buffer.byteLength)), position),
          sync: () => handle.sync(),
          close: () => handle.close(),
          truncate: (length) => handle.truncate(length),
        };
...
function actualFileSystem(): ThreadJournalFileSystem {
  return {
    ...
    readdir: async (path, options) =>
      await (await import('node:fs/promises')).readdir(path, options),
    readFile: async (path, encoding) =>
      await (await import('node:fs/promises')).readFile(path, encoding),
