...
describe('app server protocol', () => {
  it('projects items into JSON-safe cloned protocol items', () => {
    ...
    (projected.payload as { nested: { values: unknown[] } }).nested.values.push('changed');
  ...
  it('filters internal items from default protocol views', () => {
    ...
    expect(filterProtocolItems([visible, internal]).map((entry) => entry.id)).toEqual(['visible']);
    expect(
      filterProtocolItems([visible, internal], { includeInternal: true }).map((entry) => entry.id)
  ...
  it('derives thread and turn lifecycle projections from items', () => {
    ...
    expect(snapshot.turns[0]?.itemIds).toEqual(['item-1', 'item-2', 'item-3', 'item-4', 'mutated']);
  });
