    ...
    async waitForPause(): Promise<string> {
      return await Promise.race([
        ...
        new Promise<string>((_resolve, reject) => {
          setTimeout(() => reject(new Error('Cleanup did not atomically claim its path')), 1_000);
        }),
      ]);
    },
    release() {
...
describe('App Server defaults', () => {
  ...
  it('requires an explicit valid value for remote bind opt-in', () => {
    expect(readRemoteBindOptIn(undefined, 'ZEN_APP_SERVER_ALLOW_REMOTE')).toBe(false);
    ...
    expect(() => readRemoteBindOptIn('yes', 'ZEN_APP_SERVER_ALLOW_REMOTE')).toThrow(
      'ZEN_APP_SERVER_ALLOW_REMOTE must be one of: 0, 1, false, true'
    );
  });
  ...
  it('requires exactly one provided-capability or generated-handoff mode', () => {
    expect(
      readAppServerCredentialMode({
        ZEN_APP_SERVER_CAPABILITY: 'provided-capability-0123456789-abcdef-0123456789',
      })
    ).toEqual({
      type: 'provided',
    ...
    ).toThrow('Set exactly one of ZEN_APP_SERVER_CAPABILITY or ZEN_APP_SERVER_CAPABILITY_DIR');
  });
  ...
  it('publishes a unique complete handoff that a Node client claims once', async () => {
    ...
    try {
      ...
      await expect(consumeAppServerClientHandoff(published.path)).resolves.toEqual(handoff);
      await expect(consumeAppServerClientHandoff(published.path)).rejects.toMatchObject({
        code: 'ENOENT',
      });
    } finally {
  ...
  it('atomically claims the public path before cleanup ownership inspection', async () => {
    ...
    try {
      ...
      await expect(readFile(cleanupPath, 'utf8')).resolves.toContain(published.ownershipMarker);
  ...
  it('does not delete a handoff claimed concurrently before cleanup', async () => {
    ...
    try {
      ...
      await expect(readFile(claimedPath, 'utf8')).resolves.toContain(published.ownershipMarker);
      renameInterleaving.release();
  ...
  it('preserves both mismatched objects when restore races a replacement', async () => {
    ...
    try {
      ...
      await expect(readFile(published.path, 'utf8')).resolves.toBe(secondReplacement);
      await expect(readFile(cleanupPath, 'utf8')).resolves.toBe(firstReplacement);
    } finally {
      renameInterleaving.release();
      await cleanup.catch(() => undefined);
      await rm(root, { recursive: true, force: true });
