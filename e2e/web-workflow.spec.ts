...
test('uses the same-origin proxy for streamed approval, reconnect, and thread resume', async ({
  ...
  await expect(timeline.getByText('Streamed assistant progress', { exact: true })).toBeVisible();
  await expect(timeline.getByText('Approved command complete', { exact: true })).toHaveCount(0);
  fixture.progress.releaseNext();

  await expect(timeline.getByRole('button', { name: 'Approve', exact: true })).toBeVisible();
  await timeline.getByRole('button', { name: 'Approve', exact: true }).click();
  await expect(timeline.getByText('fixture shell executed', { exact: true })).toBeVisible();
  await expect(timeline.getByText('Approved command complete', { exact: true })).toBeVisible();
  ...
  await expect(timeline.getByText('Approved command complete', { exact: true })).toHaveCount(1);
});
...
test('declines a pending shell approval without executing the fixture tool', async ({ page }) => {
  ...
  await expect(timeline.getByRole('button', { name: 'Decline', exact: true })).toBeVisible();
  await timeline.getByRole('button', { name: 'Decline', exact: true }).click();
  await expect(timeline.getByText('Tool call denied by policy: approval declined')).toBeVisible();
  await expect(
    timeline.getByText('Declined command was not executed', { exact: true })
  ).toBeVisible();
