...
export function codeStateFromItems(
  items: readonly CanonicalItem[],
): Record<string, JsonValue> {
  const values: Record<string, JsonValue> = Object.create(null) as Record<
    string,
    JsonValue
  >;
  for (const item of items) {
    if (item.type === "code_state")
      values[item.key] = structuredClone(item.value);
  }
  return values;
}

export function validateCodeStateWrite(
  values: Record<string, JsonValue>,
  key: string,
  value: JsonValue,
): void {
  if (key.length === 0 || key.length > 160)
    throw new Error("store key must contain 1-160 characters");
  const encoded = JSON.stringify(value);
  if (
    encoded === undefined ||
    Buffer.byteLength(encoded) > MAX_CODE_STATE_VALUE_BYTES
  )
  ...
  if (
    Object.keys(next).length > MAX_CODE_STATE_KEYS ||
    Buffer.byteLength(JSON.stringify(next)) > MAX_CODE_STATE_BYTES
  )
