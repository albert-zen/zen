...
function renderShellTimelineRow(row: Extract<TimelineRow, { readonly type: 'shell' }>): string {
  ...
  return [`Shell ${status}: ${row.command}`, details.length > 0 ? details.join(' | ') : undefined]
    .filter((entry): entry is string => Boolean(entry))
    .join(' | ');
}
