...
export class FileThreadJournal implements ThreadJournal {
  ...
  async replay(): Promise<readonly ThreadJournalReplay[]> {
    ...
    return await Promise.all(
      entries.filter((entry) => entry.isFile()).map((entry) => this.replayFile(entry.name))
    );
  }
  ...
  async close(): Promise<void> {
    ...
    const results = await Promise.allSettled(
      [...this.writers.entries()].map(async ([, writer]) => {
        await writer.tail;
    ...
    if (failures.length > 0)
      throw new AggregateError(failures, 'Failed to close one or more thread journals');
  }

  private enqueue(
    threadId: string,
    operation: ThreadJournalError['operation'],
    action: (writer: ThreadWriter) => Promise<void>
  ): Promise<void> {
    if (this.closed)
      return Promise.reject(
        new ThreadJournalError(threadId, operation, new Error('Journal is closed'))
      );
    ...
    const operationPromise = writer.tail.then(async () => {
      ...
      try {
        ...
        const failure =
          cause instanceof ThreadJournalError
            ? cause
            : new ThreadJournalError(threadId, operation, cause);
  ...
  private async replayFile(name: string): Promise<ThreadJournalReplay> {
    ...
    if (!threadId)
      return {
        type: 'failure',
        path,
        error: new ThreadJournalCorruptionError(path, 0, 'invalid journal filename'),
      };
    try {
      ...
      if (!isCreatedForThread(items[0], threadId))
        throw new ThreadJournalCorruptionError(
          path,
          1,
          'thread.created does not match filename thread id'
        );
      ...
      return {
        ...
        threadId,
        error:
          cause instanceof ThreadJournalCorruptionError
            ? cause
            : new ThreadJournalCorruptionError(path, 0, readMessage(cause)),
      };
    }
  }
  ...
  private pathFor(threadId: string): string {
    return join(this.dir, fileNameFor(threadId));
  }
}

const nodeFileSystem: ThreadJournalFileSystem = { mkdir, readdir, readFile, open };

function encodeRecord(item: Item): Buffer {
  return Buffer.from(`${JSON.stringify({ version: JOURNAL_VERSION, item })}\n`, 'utf8');
}
function decodeRecord(line: string, path: string, recordNumber: number): Item {
  ...
  try {
    ...
    throw new ThreadJournalCorruptionError(path, recordNumber, 'invalid JSON');
  }
  if (!isRecord(value) || value.version !== JOURNAL_VERSION || !isItem(value.item))
    throw new ThreadJournalCorruptionError(path, recordNumber, 'invalid versioned Item envelope');
  return value.item;
}
function assertCreatedItem(threadId: string, item: Item): void {
  if (item.type !== 'thread.created' || !isCreatedForThread(item, threadId))
    throw new Error('Thread journal creation requires a matching thread.created Item');
}
function isCreatedForThread(item: Item, threadId: string): boolean {
  return isRecord(item.payload) && item.payload.threadId === threadId;
}
...
function fileNameFor(threadId: string): string {
  if (threadId.length === 0) throw new Error('Thread id must not be empty');
  return `thread-${Buffer.from(threadId, 'utf8').toString('base64url')}.jsonl`;
}
function decodeFileName(name: string): string | undefined {
  const match = /^thread-([A-Za-z0-9_-]+)\.jsonl$/.exec(name);
  if (!match) return undefined;
  const id = Buffer.from(match[1], 'base64url').toString('utf8');
  return id.length > 0 && fileNameFor(id) === name ? id : undefined;
}
async function writeAll(handle: FileHandle, line: Buffer): Promise<void> {
  ...
  while (offset < line.byteLength) {
    ...
    if (bytesWritten <= 0) throw new Error('File write accepted zero bytes');
    offset += bytesWritten;
  }
}
